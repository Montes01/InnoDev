...
export const Header = ({ children }: Props) => {
  ...
  if (StickyButtons) {
    if (isFixed) StickyButtons.className += newStyle;
    else
  ...
  return (
    <header className="max-w-[1000px] border-x border-[#aaa] z-20 w-full relative flex flex-col top-0 items-center justify-center gap-3 h-40 p-5 bg-primary">
      {children}
    </header>
  );
};
